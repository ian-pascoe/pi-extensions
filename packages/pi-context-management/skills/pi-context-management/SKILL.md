...
5. Other extensions' compaction summaries are ignored with a warning naming the extension; manual and threshold compaction never run them. For checkpoint or journal write failure, fix storage and reopen the persisted session; `/reload` alone is insufficient. Verify acknowledged Notes and the selected checkpoint: a failed append leaves the prior Context Window selected, while a post-append refresh failure preserves the committed checkpoint.
