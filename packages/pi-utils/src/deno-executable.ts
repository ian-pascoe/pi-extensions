...
async function npmEntry(path: string): Promise<boolean> {
  if (basename(path) !== "bin.cjs") return false;
  try {
    // npm aliases retain the published identity but use a different directory name.
    const manifest: unknown = JSON.parse(await shimText(join(dirname(path), "package.json")));
    return (
...
async function shimText(path: string): Promise<string> {
 ...
 */
export async function resolveDenoExecutable(candidate: string): Promise<string | undefined> {
  if (!(await executable(candidate))) return undefined;
  let entry: string;
  try {
  ...
  if (!(await npmEntry(entry))) {
    ...
    try {
      ...
      if (error instanceof Error && "code" in error) {
        if (["EACCES", "EPERM"].includes(String(error.code))) return candidate;
        if (["ENOENT", "ENOTDIR"].includes(String(error.code))) return undefined;
      }
      throw error;
    }
    ...
    const native =
      process.platform === "win32"
        ...
        : null;
    if (native) return executable(resolve(dirname(entry), native[1]!.replaceAll("\\", "/")));
    ...
    if (!(await npmEntry(entry))) return candidate;
  }
