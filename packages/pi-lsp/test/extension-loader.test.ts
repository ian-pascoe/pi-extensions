...
test("loads the source entrypoint without starting a language server", async () => {
  const cwd = await mkdtemp(resolve(tmpdir(), "pi-lsp-cwd-"));
  const agentDirectory = await mkdtemp(resolve(tmpdir(), "pi-lsp-agent-"));
  try {
