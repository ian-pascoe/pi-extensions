---
name: pi-lsp
description: Configure or diagnose Pi LSP when routing, framework SDK compatibility, Deno dependencies, lint helpers, diagnostics, managed installation, cancellation, updates, or lsp settings fail.
license: MIT
---
...
5. For framework, Deno, or companion failures, read the README's **Frameworks, Deno, and Lint Companions** section. Verify the selected project dependencies, compatible SDK/helper, and no-write policy before changing acquisition or runtime configuration. A publishing-trust failure requires an explicit security decision, not a verification bypass.
6. If Server Definitions changed, reload Pi. Enablement commands apply immediately. For an unavailable Server Instance, use the `restart` tool operation only when recovery is authorized.
7. Repeat status, capabilities, and diagnostics. Finish when the representative file reaches the intended Server Instance and operation, or an exact unsupported capability is evidenced.
