...
Project Context governs only project-scoped AGENTS instructions and skills. Child sessions retain user instructions and skills plus configured settings, extensions, providers, and tools regardless of Project Context; the recursive `pi-minimal-subagents` entrypoint is excluded. A configured `pi-codex-conversion` adapter replaces a complete Launch Contract tool group only when that group is granted, including the `"read"` preset's complete discovery bundle. Because Codex performs discovery through `exec_command`, the adapted preset is not an enforced read-only boundary. Status reports the effective set without mutating the Launch Contract.

Configured Context Management tools are session-maintenance capabilities, added to each Child Agent's effective runtime tools independently of ordinary-tool grants and ceilings. New, nested, and reopened Child Sessions retain these tools even with no ordinary tools; removing the extension removes those additions. Notes and History keep their existing session-branch isolation and inheritance semantics.
