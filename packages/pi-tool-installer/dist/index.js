...
export class ToolInstaller {
    ...
    async npmVersions(name, options) {
        ...
        if (!response.ok)
            throw new Error(`Cannot resolve ${name}: HTTP ${response.status}`);
        const value = await response.json();
        if (!Value.Check(NpmVersionsSchema, value) ||
            Object.entries(value.versions).some(([version, manifest]) => manifest.name !== name || manifest.version !== version))
    ...
    async list() {
        ...
        const installations = await Promise.all(files
    ...
    async concreteTool(selector, version, options) {
        ...
        if (!response.ok)
            throw new Error(`Cannot fetch published checksum: HTTP ${response.status}`);
        const checksum = (await response.text()).trim();
        if (!/^[a-fA-F0-9]{64}$/.test(checksum))
            throw new Error("Invalid published SHA-256 checksum");
    ...
    async acquire(request, options, previous) {
        ...
        for (const [key, selector] of Object.entries(request.requirements)) {
            ...
            if (scopedBackend) {
                ...
                const namespace = join(this.directory, scopedBackend, createHash("sha256").update(graph).digest("hex"));
                installEnvironment = {
                    ...environment,
