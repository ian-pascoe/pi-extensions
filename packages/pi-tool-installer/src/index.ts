...
const InstallationSchema = Type.Object({
  ...
  components: Type.Record(
    IdSchema,
    Type.Object({ selector: SelectorSchema, version: VersionSchema, directory: Type.String() }),
...
function matches(installation: ManagedInstallation | undefined, request: ToolRequest): boolean {
  return (
    JSON.stringify(
      Object.entries(installation?.components ?? {}).map(([key, value]) => [key, value.selector]),
    ) === JSON.stringify(Object.entries(request.requirements))
  );
}
...
async function validateDirectories(
  installation: ManagedInstallation,
  directory: string,
  contexts: Static<typeof ContextSchema>[] = [],
): Promise<void> {
...
export class ToolInstaller {
  ...
  private async selection(id: string): Promise<Selection | undefined> {
    ...
    if (
      !Value.Check(SelectionSchema, value) ||
  ...
  async ensure(
    ...
    if (existing && matches(existing, request)) return existing;
    ...
    if (!options.allowDownload) {
      try {
        ...
        if (Value.Check(FileErrorSchema, error)) throw unavailable;
        throw error;
      }
    }
    return this.withInstallationLock(options, async (signal) => {
      const installed = await this.installed(request.id);
      if (installed && matches(installed, request)) return installed;
      ...
      if (!options.allowDownload) throw unavailable;
  ...
  private async reuse(
  ...
  ): Promise<ManagedInstallation | undefined> {
    let files: string[];
    try {
    ...
    for (const file of files.sort()) {
      ...
      if (
        !requirements.every(([key, selector], index) => {
      ...
      const context =
        selection.contexts?.[requirements.length - 1] ??
        ...
        return await this.publish(
          {
            ...
            components: Object.fromEntries(
              requirements.map(([key], index) => [key, components[index]!]),
            ),
            binDirectories: context.binDirectories,
            environment: context.environment,
          },
          options,
          selection.contexts?.slice(0, requirements.length),
        );
      } catch (error) {
        if (Value.Check(FileErrorSchema, error)) continue;
        throw error;
      }
    }
    return undefined;
  ...
  private async helper(options: InstallationOptions): Promise<string> {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Cannot discover mise: HTTP ${response.status}${rateLimit === "0" ? ". GitHub API rate limit exhausted; retry later or supply GITHUB_TOKEN." : ""}`,
      );
    }
  ...
  private async run(
    ...
    return new Promise((resolveResult, reject) => {
      ...
      child.stderr.setEncoding("utf8").on("data", (data: string) => {
        ...
        for (const line of lines) if (line.trim()) options.onProgress?.(line.trim());
      });
      const cancel = () => {
        if (!child.pid) return;
  ...
  private async acquire(
    ...
    const contexts: Static<typeof ContextSchema>[] = [];
    const tools: string[] = [];
    for (const [key, selector] of Object.entries(request.requirements)) {
      ...
      if (selector.startsWith("pipx:")) {
        ...
        if (!python) throw new Error(`${selector} requires a preceding managed Python runtime`);
      ...
      const value: unknown = JSON.parse(
        await this.run(
          helper,
          ["env", "--json", ...prerequisites, concrete],
          installEnvironment,
          options,
        ),
      );
