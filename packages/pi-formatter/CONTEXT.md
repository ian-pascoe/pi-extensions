...
- **Formatter Definition** — a configured command, file matching policy, workspace-root policy, environment, and stable formatter ID. A project Formatter Definition completely replaces a global definition with the same ID; `null` or an invalid project replacement shadows the global definition.
